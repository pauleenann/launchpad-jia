    ...
    return (
        <div style={{display:'grid', gridTemplateColumns:'1fr 30%', gap:'20px', marginTop:'30px'}}>
                    ...
                    <button style={{ width: "fit-content", background: "black", color: "#fff", border: "1px solid #E9EAEB", padding: "8px 16px", borderRadius: "60px", whiteSpace: "nowrap", display:'flex', alignItems:'center', gap:'8px', cursor: 'pointer'}}>
                        ...
                        {suggestedQuestions.map((suggestion, index)=>(
                            <div key={index} style={{display: 'flex', alignItems:'center', justifyContent:'space-between', marginTop:'10px'}}>
                                ...
                                <button style={{padding: '5px 15px', border:'1px solid gray', borderRadius:'20px', background:'white'}}>Add</button>
                            </div>
                        ))}
                </CareerStepHolder>
            </div>
            ...
            <div>
                ...
                label={'Tips'}
                customHeader={''}>
                    <p style={{fontWeight:'500'}}><strong style={{color:'#2a2a2a'}}>Add a Secret Prompt</strong> to fine-tune how Jia scores and evaluates submitted CVs..</p>
                    <p style={{fontWeight:'500'}}><strong style={{color:'#2a2a2a'}}>Add Pre-Screening Questions</strong> to collect key details such as notice period, work setup, or salary expectations to guide your review and candidate discussions.</p>
                </CareerStepHolder>
            </div>
        </div>
    )
